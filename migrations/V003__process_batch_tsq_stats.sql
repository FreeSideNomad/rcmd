-- V003: Update sp_refresh_batch_stats for process batch TSQ detection
--
-- For PROCESS batches, the stored procedure now detects processes that are
-- blocked due to having a command in IN_TROUBLESHOOTING_QUEUE status.
...
-- - in_troubleshooting_count: processes blocked by TSQ commands
-- - in_progress: calculated as total - completed - canceled - blocked
...
BEGIN
    ...
    -- For process batches, batch completes when all processes are completed, failed, or blocked in TSQ
    ...
    SET completed_count = v_completed,
        ...
        status = CASE
            WHEN v_is_complete AND (v_canceled > 0 OR v_in_tsq > 0) THEN 'COMPLETED_WITH_FAILURES'
